...
| F-03 | 検索 (`search`) | BM25 + Chroma のハイブリッド検索と簡易 rerank で上位を返す。Chroma 不在時は BM25 単独。α/TopK の初期値は `alpha=0.5`, `top_k=5`, `top_k_bm25=10`, `top_k_chroma=10`。 | F-02 (インデックス) | MVP(v0) |
...
## 4. フェーズ分け
- MVP (v0): F-00〜F-06 の基本挙動を実装し、`optimize` は全 level 未サポート例外で返す。チャンク戦略 (`chunk_size=512`, `overlap=32`) と検索パラメータ (`alpha=0.5`, `top_k=5`, `top_k_bm25=10`, `top_k_chroma=10`) は固定。デフォルトの LLM/Embedding は OpenAI（`OPENAI_API_KEY` 必須）を利用。
